"""Entry point for cMLX menubar app.
...
def _get_crash_log_path() -> Path:
    ...
    app_support = Path.home() / "Library" / "Application Support" / "cMLX"
...
def _check_os_version() -> None:
    ...
    if major < 15:
        _show_error_dialog(
            "macOS 15.0 Required",
            f"cMLX requires macOS 15.0 (Sequoia) or later.\n\n"
            f"Your system is running macOS {mac_ver}.\n\n"
            f"MLX >= 0.29.2 requires macOS 15.0+. "
            f"Please update your operating system to use cMLX.",
        )
        sys.exit(1)
...
try:
    ...
    if "mlx" in error_str.lower() or "libmlx" in exc_text.lower():
        _show_error_dialog(
            ...
            "cMLX failed to load the MLX framework.\n\n"
            "This usually means your macOS version is too old. "
            "cMLX requires macOS 15.0 (Sequoia) or later.\n\n"
