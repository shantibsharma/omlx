"""
Auto-update module for cMLX macOS app.
...
class AppUpdater:
    ...
    STAGED_APP_NAME = ".cMLX-update.app"
        ...
        try:
            ...
            if not self.is_writable(app_path):
                raise UpdateError(
                    ...
                    "Please move cMLX.app to a writable location."
                )

            # Download DMG
            self._progress("Downloading update...")
            tmp_dir = Path(tempfile.mkdtemp(prefix="cmlx-update-"))
            dmg_path = tmp_dir / f"cMLX-{self.version}.dmg"
            ...
            # Find cMLX.app in mounted volume
    ...
    def _find_app_in_volume(mount_point: Path) -> Path:
        ...
        raise UpdateError("cMLX.app not found in DMG")
