"""Server process management for cMLX menubar app."""
...
def get_bundled_python() -> str:
    ...
    # Menubar launcher case: sys.executable may be .../Contents/MacOS/cMLX.
...
class ServerManager:
    """Manages the cMLX server process lifecycle."""
    ...
    def _is_cmlx_server(self) -> bool:
        """Check if the process on the port is an cMLX server."""
        try:
            session = requests.Session()
            session.trust_env = False
    ...
    def adopt(self) -> bool:
        """Adopt an externally-running cMLX server without owning the process."""
        if not self._is_cmlx_server():
            return False
        ...
        logger.info(f"Adopted external cMLX server on port {self.config.port}")
        return True
    ...
    def _do_start(self) -> Union[bool, PortConflict]:
        ...
        if self._is_port_in_use():
            is_cmlx = self._is_cmlx_server()
            pid = self._find_port_owner_pid()
            return PortConflict(pid=pid, is_cmlx=is_cmlx)
        ...
        try:
            ...
            cmd = [python_exe, "-m", "cmlx.cli"] + args
