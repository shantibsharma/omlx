"""
cMLX Native Menubar Application using PyObjC.

A native macOS menubar app for managing the cMLX LLM inference server.
"""
...
def _find_matching_dmg(assets: list[dict]) -> str | None:
    ...
    DMG filenames follow the pattern: cMLX-0.2.10-macos15-sequoia_260210.dmg
...
class CMLXAppDelegate(NSObject):
    """Main application delegate for cMLX menubar app."""

    def init(self):
        self = objc.super(CMLXAppDelegate, self).init()
        if self is None:
            return None
    ...
    def _show_fatal_error_and_quit(self, message: str):
        ...
        alert.setMessageText_("cMLX Failed to Launch")
    ...
    def _doFinishLaunching(self):
        ...
        logger.info("cMLX menubar app launched successfully")
    ...
    def _get_resources_dir(self) -> Path:
        ...
        # App bundle: __file__ is Resources/cmlx_app/app.py → parent.parent = Resources/
        ...
        if bundle and bundle.resourcePath():
            ...
            if (res / "navbar-logo-dark.svg").exists():
                return res
        # Development fallback: cmlx/admin/static/
        dev_path = (
            Path(__file__).parent.parent.parent / "cmlx" / "admin" / "static"
        )
        if dev_path.exists():
            return dev_path
    ...
    def _update_menubar_icon(self):
        ...
        if icon:
            ...
            self.status_item.setTitle_("cMLX")
    ...
    def _check_for_updates(self):
        ...
        try:
            # GitHub Releases API
            resp = requests.get(
                "https://api.github.com/repos/shantibsharma/cmlx/releases/latest",
                timeout=5,
            )
    ...
    def openUpdate_(self, sender):
        ...
        alert.setMessageText_(
            f"Update to cMLX {self._update_info['version']}?"
        )
    ...
    def _open_update_browser(self):
        ...
        url = (
            ...
            else "https://github.com/shantibsharma/cmlx/releases"
        )
        webbrowser.open(url)
    ...
    def _start_auto_update(self):
        ...
        if not AppUpdater.is_writable(app_path):
            ...
            alert.setInformativeText_(
                f"cMLX does not have write permission to {app_path.parent}.\n\n"
    ...
    def _get_status_display(self):
        ...
        if status == ServerStatus.RUNNING:
            return "● cMLX Server is running", NSColor.systemGreenColor()
        elif status == ServerStatus.STARTING:
            return "● cMLX Server is starting...", NSColor.systemOrangeColor()
        elif status == ServerStatus.STOPPING:
            return "● cMLX Server is stopping...", NSColor.systemOrangeColor()
        elif status == ServerStatus.UNRESPONSIVE:
            return "● cMLX Server is not responding", NSColor.systemOrangeColor()
        elif status == ServerStatus.ERROR:
            err = self.server_manager.error_message or "Unknown error"
            return f"● {err}", NSColor.systemRedColor()
        else:
            return "● cMLX Server is stopped", NSColor.secondaryLabelColor()
    ...
    def _handle_port_conflict(self, conflict: PortConflict) -> None:
        ...
        if conflict.is_cmlx:
            alert.setMessageText_("cMLX Server Already Running")
            pid_info = f" (PID {conflict.pid})" if conflict.pid else ""
            alert.setInformativeText_(
                f"An cMLX server is already running on port "
    ...
    def showAbout_(self, sender):
        ...
        alert = NSAlert.alloc().init()
        alert.setMessageText_("About cMLX")

        try:
            from cmlx._build_info import build_number
        except ImportError:
            build_number = None
...
def main():
    ...
    delegate = CMLXAppDelegate.alloc().init()
