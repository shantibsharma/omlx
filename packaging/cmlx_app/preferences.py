"""Settings window for cMLX app settings - modern macOS native design."""
...
LAUNCH_AGENT_LABEL = "com.cmlx.app"
...
class PreferencesWindowController(NSObject):
    ...
    def showWindow(self):
        ...
        self.window.setTitle_("cMLX Settings")
        self.window.center()
        ...
        self.launch_at_login_checkbox.setTitle_("Launch cMLX at login")
    ...
    def browseBaseDir_(self, sender):
        ...
        panel.setMessage_(
            "Choose a parent folder. An .cmlx directory will be created inside it."
        )

        if panel.runModal() == 1:
            selected = str(panel.URL().path())
            new_base = str(Path(selected) / ".cmlx")
    ...
    def _apply_launch_at_login(self, enabled: bool):
        ...
        try:
            if enabled:
                ...
                if not app_path:
                    logger.warning("Cannot find cMLX.app bundle for launch agent")
                    return
