...
A critical engineering feat in `omlx` is the resolution of **Metal Stream Contention**. Since the `mlx-lm` framework utilizes a global Metal device stream, concurrent access from multiple asyncio tasks can cause segmentation faults.
- **Serialized Execution:** `omlx` isolates the generation loop (`scheduler.step`) within a dedicated `ThreadPoolExecutor(max_workers=1)`. 
...
To facilitate long-context conversations on memory-constrained hardware, `omlx` implements a block-based, tiered cache architecture inspired by vLLM.
...
- **Extensibility:** The adapter pattern (`omlx/adapter/`) makes it trivial to add support for new model families or custom output parsers.
