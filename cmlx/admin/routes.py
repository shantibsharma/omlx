...
"""Admin panel routes for cMLX server configuration.
...
def _apply_log_level_runtime(level: str) -> None:
    """Apply log level change at runtime to all cMLX loggers and handlers."""
    ...
    # Update cmlx-related loggers
    cmlx_loggers = [
        "cmlx",
        "cmlx.scheduler",
        "cmlx.paged_ssd_cache",
        "cmlx.memory_monitor",
        "cmlx.paged_cache",
        "cmlx.prefix_cache",
        "cmlx.engine_pool",
        "cmlx.model_discovery",
        "cmlx.engine_core",
        "cmlx.engine",
        "cmlx.server",
        "cmlx.admin",
    ...
    for logger_name in cmlx_loggers:
...
from cmlx._version import __version__ as _cmlx_version
templates.env.globals["version"] = _cmlx_version
...
async def logout(response: Response):
    ...
    Returns:
        JSON response with success status.
    """
    response.delete_cookie(key="cmlx_admin_session")
    return {"success": True}
...
def _load_fallback_commits(packages: dict[str, str]) -> dict:
    ...
    Tries in order:
    1. _engine_commits.json (generated by build.py, lives in cmlx package dir)
    2. pyproject.toml (dev environment, lives one level above package dir)
    """
    import json
    from pathlib import Path

    # This file is at cmlx/admin/routes.py → package dir is cmlx/
...
async def get_device_info(
    ...
    """Get device hardware info and owner_hash for cmlx.ai integration."""
...
async def check_update(
    ...
    """Check GitHub Releases for newer cMLX version (cached 24h)."""
    ...
    try:
        ...
        try:
            ...
            update_available = (
                latest_ver > Version(_cmlx_version)
