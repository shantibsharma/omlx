...
"""Benchmark execution logic for cMLX admin panel.
...
CMLX_AI_API_URL = "https://cmlx.ai/api/benchmarks"
...
async def _upload_to_cmlx_ai(run: BenchmarkRun, engine_pool: Any) -> None:
    """Upload benchmark results to cmlx.ai community benchmarks.
...
async def run_benchmark(run: BenchmarkRun, engine_pool: Any) -> None:
    ...
    try:
        ...
        # Upload results to cmlx.ai (failures don't affect benchmark status)
        try:
            await _upload_to_cmlx_ai(run, engine_pool)
        except Exception as e:
            logger.warning(f"Benchmark upload to cmlx.ai failed: {e}")
            await _send_event(run, {
                "type": "upload_done",
                "data": {
                    "owner_hash": None,
