...
"""
OpenAI-compatible API server for cMLX.
...
Usage:
    # Multi-model serving
    cmlx serve --model-dir /path/to/models --max-model-memory 32GB

    # With pinned models
    cmlx serve --model-dir /path/to/models --max-model-memory 48GB --pin llama-3b,qwen-7b

    # With MCP tools
    cmlx serve --model-dir /path/to/models --max-model-memory 32GB --mcp-config mcp.json
...
# Robust singleton to handle cases where cmlx.server is imported as both '__main__' and 'cmlx.server'
import sys

if hasattr(sys, "_cmlx_server_state"):
    _server_state = sys._cmlx_server_state
else:
...
class AdminLogFilter(logging.Filter):
    def filter(self, record):
        # 1. Suppress if the logger name starts with cmlx.admin
        if record.name.startswith("cmlx.admin"):
            return False
...
for logger_name in ("uvicorn.access", "uvicorn", "cmlx.admin"):
    l = logging.getLogger(logger_name)
    l.addFilter(AdminLogFilter())
    # Ensure cmlx.admin doesn't propagate to root if we want it completely silent
    if logger_name == "cmlx.admin":
        l.propagate = False
...
async def lifespan(app: FastAPI):
    ...
    if _server_state.global_settings is None or _server_state.engine_pool is None:
        ...
        model_dirs = os.environ.get("CMLX_MODEL_DIR", str(global_settings.model.get_model_dir(global_settings.base_path)))
        max_mem_str = os.environ.get("CMLX_MAX_MODEL_MEMORY", global_settings.model.max_model_memory)
        pinned_str = os.environ.get("CMLX_PINNED_MODELS")
        pinned_models = pinned_str.split(",") if pinned_str else None
        default_model = os.environ.get("CMLX_DEFAULT_MODEL")
    ...
    mcp_config = os.environ.get("CMLX_MCP_CONFIG")
...
def _compile_grammar_for_request(
    ...
    if compiler is None:
        if structured_outputs is not None:
            from cmlx.utils.install import get_install_method
            ...
            if method == "dmg":
                ...
                detail = (
                    ...
                    "Reinstall with: brew reinstall cmlx --with-grammar"
                )
            else:
                detail = (
                    "Structured output requires xgrammar. "
                    "Install with: pip install 'cmlx[grammar]'"
                )
...
async def create_anthropic_message(
    ...
    allowing clients that use Anthropic SDK to work with cMLX.
...
async def init_mcp(config_path: str):
    ...
    try:
        from cmlx.mcp import MCPClientManager, ToolExecutor, load_mcp_config
...
def main():
    """Run the server (use cmlx CLI instead)."""
    from .config import parse_size
    from .settings import init_settings

    parser = argparse.ArgumentParser(
        description="cMLX multi-model serving for Apple Silicon",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Multi-model serving
    python -m cmlx.server --model-dir /path/to/models --max-model-memory 32GB

    # With pinned models
    python -m cmlx.server --model-dir /path/to/models --max-model-memory 48GB --pin llama-3b,qwen-7b

    # With MCP tools
    python -m cmlx.server --model-dir /path/to/models --max-model-memory 32GB --mcp-config mcp.json

Note: Use the cmlx CLI for full feature support.
        """,
    )
    parser.add_argument(
        "--model-dir",
    ...
    if args.model_dir:
        os.environ["CMLX_MODEL_DIR"] = args.model_dir
    if args.max_model_memory:
        os.environ["CMLX_MAX_MODEL_MEMORY"] = args.max_model_memory
    if args.pin:
        os.environ["CMLX_PINNED_MODELS"] = args.pin
    if args.default_model:
        os.environ["CMLX_DEFAULT_MODEL"] = args.default_model
    ...
    if args.mcp_config:
        os.environ["CMLX_MCP_CONFIG"] = args.mcp_config
