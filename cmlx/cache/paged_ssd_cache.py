...
"""
Paged SSD Cache Manager for cMLX KV cache.
...
from cmlx.utils.memory import sync_and_clear_cache
            ...
            for i, layer_data in enumerate(cache_data):
                if (isinstance(layer_data, tuple) and len(layer_data) == 2
                    ...
                    cache_list_meta[f"layer_{i}_tq_value_fields"] = ",".join(vs._fields)
                        ...
                        and isinstance(layer_data[0], str)
                        and layer_data[0] == "__cmlx_quant_v1__"):
                    # CMLX INT8 Quantization: (qk, qv, sk, sv, bk, bv)
                    ...
                    cache_list_meta[f"layer_{i}_quant_v1"] = "1"
                        ...
                        and isinstance(layer_data[0], str)
                        and layer_data[0] == "__cmlx_fp8_v1__"):
                    # CMLX FP8 Quantization: (qk, qv, sk, sv)
                    payload = layer_data[1]
                    arrays[f"layer_{i}_qk"] = payload[0]
                    arrays[f"layer_{i}_qv"] = payload[1]
                    arrays[f"layer_{i}_sk"] = payload[2]
    ...
    def _reconstruct_cache_data(
        ...
        for i in range(num_layers):
            ...
            if cache_type == 'CacheList':
                ...
                cache_data.append(("__cmlx_quant_v1__", (qk, qv, sk, sv, bk, bv)))
            elif file_metadata and f"layer_{i}_fp8_v1" in file_metadata:
                # CMLX FP8 Quantization
                qk = arrays[f"layer_{i}_qk"]
                qv = arrays[f"layer_{i}_qv"]
                sk = arrays[f"layer_{i}_sk"]
                sv = arrays[f"layer_{i}_sv"]
                cache_data.append(("__cmlx_fp8_v1__", (qk, qv, sk, sv)))
            else:
                keys_key = f"layer_{i}_keys"
                values_key = f"layer_{i}_values"
