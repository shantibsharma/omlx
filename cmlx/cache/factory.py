...
Note: cMLX only supports paged SSD-based caching. Memory KV cache is managed
by mlx-lm's BatchGenerator. When paged SSD cache is disabled, no cMLX caching
is performed.
"""
...
class CacheConfig:
    ...
    Note: cMLX only supports paged SSD-based caching via BlockAwarePrefixCache.
...
class CacheFactory:
    ...
    based on a unified CacheConfig. cMLX only supports paged SSD-based caching
    ...
    def create_full_cache_stack(
        ...
        is disabled (cMLX only supports paged SSD-based caching).
