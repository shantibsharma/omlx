...
class OpenClawIntegration(Integration):
    ...
    def configure(
        ...
        def updater(config: dict) -> None:
            ...
            config["models"]["providers"]["cmlx"] = provider_config
            ...
            if model:
                ...
                config["agents"]["defaults"]["model"]["primary"] = f"cmlx/{model}"
    ...
    def launch(
        ...
        **kwargs,
        ...
        if not self._is_onboarded():
            print("Setting up OpenClaw with cMLX...")
            print(f"  Model: {model}")
            subprocess.run(
                [
                    bin_name,
