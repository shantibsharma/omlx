...
CONFIG_SEARCH_PATHS = [
    "./mcp.json",
    "./mcp.yaml",
    "~/.config/cmlx/mcp.json",
    "~/.config/cmlx/mcp.yaml",
]

# Environment variable for config path
CONFIG_ENV_VAR = "CMLX_MCP_CONFIG"
...
def load_mcp_config(path: Optional[Union[str, Path]] = None) -> MCPConfig:
    ...
    2. CMLX_MCP_CONFIG environment variable
    3. ./mcp.json or ./mcp.yaml (current directory)
    4. ~/.config/cmlx/mcp.json or mcp.yaml
...
def validate_config(data: Dict[str, Any]) -> MCPConfig:
    ...
    # Support both cMLX ("servers") and Claude Desktop ("mcpServers") format
