...
"""
Global settings management for cMLX.
...
Usage:
    from cmlx.settings import init_settings, get_settings
...
# Default base path
DEFAULT_BASE_PATH = Path.home() / ".cmlx"
...
class ModelSettings:
    ...
    model_dirs: list[str] = field(default_factory=list)  # [] means ~/.cmlx/models
...
class CacheSettings:
    ...
    enabled: bool = True
    ssd_cache_dir: str | None = None  # None means ~/.cmlx/cache
...
class ClaudeCodeSettings:
    ...
    # Mode: "cloud" = native claude.ai subscription, "local" = route through cmlx.
    # Default is "cloud" so upgrades don't silently route traffic to cmlx.
    mode: str = "cloud"
...
class GlobalSettings:
    ...
    def load(
        ...
        Args:
            base_path: Base directory for cMLX (default: ~/.cmlx).
    ...
    def _apply_env_overrides(self) -> None:
        """Apply CMLX_* environment variable overrides."""
        # Server settings
        if host := os.getenv("CMLX_HOST"):
            self.server.host = host
        if port := os.getenv("CMLX_PORT"):
            try:
                self.server.port = int(port)
            except ValueError:
                logger.warning(f"Invalid CMLX_PORT value: {port}")
        if log_level := os.getenv("CMLX_LOG_LEVEL"):
            self.server.log_level = log_level

        # Model settings
        if model_dir := os.getenv("CMLX_MODEL_DIR"):
            dirs = [d.strip() for d in model_dir.split(",") if d.strip()]
            self.model.model_dirs = dirs
            self.model.model_dir = dirs[0] if dirs else None
        if max_model_memory := os.getenv("CMLX_MAX_MODEL_MEMORY"):
            self.model.max_model_memory = max_model_memory

        # Memory enforcement settings
        if available_system_memory := os.getenv("CMLX_AVAILABLE_SYSTEM_MEMORY"):
            self.memory.available_system_memory = available_system_memory
        if max_process_memory := os.getenv("CMLX_MAX_PROCESS_MEMORY"):
            self.memory.max_process_memory = max_process_memory

        # Scheduler settings
        max_concurrent = os.getenv("CMLX_MAX_CONCURRENT_REQUESTS") or os.getenv(
            "CMLX_MAX_NUM_SEQS"
        )
        if max_concurrent:
            try:
                ...
                logger.warning(
                    f"Invalid CMLX_MAX_CONCURRENT_REQUESTS value: {max_concurrent}"
                )

        # Cache settings
        if cache_enabled := os.getenv("CMLX_CACHE_ENABLED"):
            self.cache.enabled = cache_enabled.lower() in ("true", "1", "yes")
        if ssd_cache_dir := os.getenv("CMLX_SSD_CACHE_DIR"):
            self.cache.ssd_cache_dir = ssd_cache_dir
        if ssd_cache_max := os.getenv("CMLX_SSD_CACHE_MAX_SIZE"):
            self.cache.ssd_cache_max_size = ssd_cache_max
        if initial_blocks := os.getenv("CMLX_INITIAL_CACHE_BLOCKS"):
            try:
                self.cache.initial_cache_blocks = int(initial_blocks)
            except ValueError:
                logger.warning(
                    f"Invalid CMLX_INITIAL_CACHE_BLOCKS value: {initial_blocks}"
                )

        # Auth settings
        if api_key := os.getenv("CMLX_API_KEY"):
            self.auth.api_key = api_key

        # MCP settings
        if mcp_config := os.getenv("CMLX_MCP_CONFIG"):
            self.mcp.config_path = mcp_config

        # HuggingFace settings
        if hf_endpoint := os.getenv("CMLX_HF_ENDPOINT"):
            self.huggingface.endpoint = hf_endpoint

        # ModelScope settings
        if ms_endpoint := os.getenv("CMLX_MS_ENDPOINT"):
            self.modelscope.endpoint = ms_endpoint

        # Logging settings
        if log_dir := os.getenv("CMLX_LOG_DIR"):
            self.logging.log_dir = log_dir
        if retention_days := os.getenv("CMLX_LOG_RETENTION_DAYS"):
            try:
                self.logging.retention_days = int(retention_days)
            except ValueError:
                logger.warning(f"Invalid CMLX_LOG_RETENTION_DAYS: {retention_days}")
