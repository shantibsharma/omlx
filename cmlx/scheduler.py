...
"""
Scheduler for cMLX continuous batching.
...
class SchedulerConfig:
    ...
    # paged SSD cache settings (cMLX only supports paged SSD-based caching)
    # When paged_ssd_cache_dir is set, cMLX stores KV cache on paged SSD for prefix reuse.
    # When None, no cMLX caching (mlx-lm BatchGenerator manages KV internally).
...
class Scheduler:
    ...
    def __init__(
        ...
        # paged SSD cache for KV state persistence (cMLX only supports paged SSD-based caching)
        ...
        if self.config.paged_ssd_cache_dir:
            ...
            logger.info("cMLX cache disabled (mlx-lm BatchGenerator manages KV internally)")
