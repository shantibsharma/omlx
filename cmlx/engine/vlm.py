...
- Full compatibility with cMLX's tiered KV cache and boundary snapshots
...
def _patch_video_processor_bug():
    ...
    cMLX does not support video input. Without torchvision, transformers'
...
class VLMBatchedEngine(BaseEngine):
    ...
    async def start(self) -> None:
        ...
        def _load_vlm_sync():
            ...
            # cMLX does not support video input, so we skip video processing.
    ...
    def _prepare_vision_inputs(
        ...
        # Build per-message placeholders in cMLX so image-bearing turns always
