...
Commands:
    cmlx serve --model-dir /path/to/models    Start multi-model server
...
def serve_command(args):
    ...
    print(f"\033[33mcMLX - LLM inference, optimized for your Mac\033[0m")
    print(f"\033[33m├─ https://github.com/shantibsharma/cmlx\033[0m")
    if build_number:
    ...
    # Set cmlx loggers
    for name in ["cmlx", "cmlx.scheduler", "cmlx.paged_ssd_cache",
                 "cmlx.memory_monitor", "cmlx.paged_cache", "cmlx.prefix_cache",
                 "cmlx.engine_pool", "cmlx.model_discovery"]:
    ...
    if mcp_config:
        ...
        os.environ["CMLX_MCP_CONFIG"] = mcp_config
    ...
    if args.no_cache:
        print("Mode: Multi-model serving (no cMLX cache, mlx-lm BatchGenerator only)")
...
def launch_command(args):
    """Launch an external tool integrated with cMLX."""
    import requests
    ...
    # Check if cMLX server is running
    base_url = f"http://{host}:{port}"
    try:
        resp = requests.get(f"{base_url}/health", timeout=3)
        resp.raise_for_status()
    except Exception:
        print(f"cMLX server is not running at {base_url}")
        print("Start the server first: cmlx serve")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="cmlx: Production-ready LLM server for Apple Silicon",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  cmlx serve mlx-community/Llama-3.2-3B-Instruct-4bit --port 8000
  cmlx launch codex --model qwen3.5
        """,
    )
...
""",
    ...
    serve_parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory containing model subdirectories (default: ~/.cmlx/models)",
    )
    serve_parser.add_argument(
        "--max-model-memory",
        type=str,
    ...
    serve_parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory for paged SSD cache storage (enables cMLX prefix cache)",
    )
    serve_parser.add_argument(
    ...
    serve_parser.add_argument(
        "--no-cache",
        action="store_true",
        help="Disable cMLX paged SSD cache. mlx-lm BatchGenerator still manages KV states internally.",
    )
    serve_parser.add_argument(
    ...
    serve_parser.add_argument(
        ...
        type=str,
        default=None,
        help="Base directory for cMLX data (default: ~/.cmlx)",
    )
    serve_parser.add_argument(
        "--api-key",
        type=str,
    ...
    launch_parser = subparsers.add_parser(
        "launch",
        help="Launch an external tool with cMLX integration",
        description="Configure and launch external coding tools (Codex, OpenCode, OpenClaw) "
        "to use the running cMLX server.",
    )
    ...
    launch_parser.add_argument(
        ...
        type=str,
        default=None,
        help="cMLX server host (default: from settings or 127.0.0.1)",
    )
    launch_parser.add_argument(
        "--port",
        type=int,
        default=None,
        help="cMLX server port (default: from settings or 8000)",
    )
    launch_parser.add_argument(
        "--api-key",
        type=str,
        default=None,
        help="API key for cMLX server authentication",
    )
    launch_parser.add_argument(
        "--tools-profile",
        type=str,
