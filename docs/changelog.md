...
- **True Singleton State**: Resolved a critical "double-import" bug where state was inconsistent between modules. State is now a global singleton protected via `sys._omlx_server_state`.
...
### **4. Logging & UX**
- **Admin Log Suppression**: Implemented a strict `AdminLogFilter` that silences all `/admin/*` and `omlx.admin` traffic from the console. Users now only see relevant model and API logs.
...
---
*Status: oMLX is now M4 Pro optimized, stable for agentic loops, and prefix-cache ready.*
