#!/bin/bash
# Setup script for CMLX development on macOS (M4 Pro optimized)
...
# 5. Install CMLX in editable mode (includes native C++/FP8 extensions)
echo "🏗️ Building and installing CMLX with Native FP8 acceleration..."
cd "$PROJECT_ROOT"
...
if [ -d "$PROJECT_ROOT/vllm-metal" ]; then
    ...
    PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 pip install -e "$PROJECT_ROOT/vllm-metal" 2>&1 || {
        ...
        echo "  ℹ️  cMLX will still work without vllm-metal (using standard MLX attention)."
    }
else
    ...
    PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 pip install -e "$PROJECT_ROOT/vllm-metal" 2>&1 || {
        echo "  ⚠️  vllm-metal install failed. cMLX will use standard MLX attention as fallback."
    }
fi
