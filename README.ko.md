...
<p align="center"><b>Mac에 최적화된 LLM 추론 서버</b><br>연속 배칭과 계층형 KV 캐시, 메뉴 바에서 바로 관리.</p>
...
> *써본 LLM 서버마다 편의성과 제어 중 하나를 골라야 했습니다. 자주 쓰는 모델은 메모리에 고정하고, 무거운 모델은 필요할 때 자동으로 교체하고, 컨텍스트 제한도 설정하면서 — 이 모든 걸 메뉴 바에서 관리하고 싶었습니다.*
>
> *oMLX는 Hot 메모리 캐시와 Cold SSD 캐시 두 계층에 걸쳐 KV 캐시를 유지합니다. 대화 중간에 컨텍스트가 바뀌어도 모든 과거 컨텍스트가 캐시에 남아 요청 간에 재사용됩니다. Claude Code 같은 도구로 실제 코딩 작업을 할 때 로컬 LLM이 진짜 쓸만해집니다. 그래서 만들었습니다.*
...
Claude Code에서 작은 컨텍스트 모델을 실행하기 위한 컨텍스트 스케일링을 지원합니다. 보고되는 토큰 수를 스케일링하여 자동 Compact가 적절한 타이밍에 트리거되고, 긴 프리필 동안 읽기 타임아웃을 방지하는 SSE keep-alive를 제공합니다.
...
관리자 패널에서 로드된 모델과 직접 채팅합니다. 대화 기록, 모델 전환, 다크 모드, 추론 모델 출력을 지원합니다.
