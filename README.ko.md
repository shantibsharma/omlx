...
<p align="center">
  ...
  <a href="https://cmlx.ai/benchmarks">벤치마크</a> ·
...
<p align="center">
  <img src="docs/images/cmlx_dashboard.png" alt="cMLX 관리자 대시보드" width="800">
</p>
...
>
> *cMLX는 Hot Cache-메모리와 Cold Cache-SSD의 두 계층에 걸쳐 KV캐시를 유지합니다. 대화 중간에 컨텍스트가 바뀌어도 모든 과거 컨텍스트가 캐시에 남습니다. Claude Code 같은 도구로 실제 코딩 작업을 해보셨나요? cMLX와 함께라면 가능합니다.*

## 설치

### macOS 앱

[Releases](https://github.com/shantibsharma/cmlx/releases)에서 `.dmg`를 다운로드하고, Applications에 드래그하면 끝입니다. 앱 내 자동 업데이트를 지원하므로 이후 업그레이드는 클릭 한 번이면 됩니다. macOS 앱은 `cmlx` CLI 명령어를 포함하지 않습니다. 터미널 사용이 필요하면 Homebrew 또는 소스에서 설치하세요.
...
Applications 폴더에서 cMLX를 실행하세요. 환영 화면에서 세 단계만 따라하면 됩니다 — 모델 디렉토리 설정, 서버 시작, 첫 모델 다운로드. 끝입니다.

<p align="center">
  <img src="docs/images/Screenshot 2026-02-10 at 00.36.32.png" alt="cMLX 환영 화면" width="360">
  <img src="docs/images/Screenshot 2026-02-10 at 00.34.30.png" alt="cMLX 메뉴 바" width="240">
</p>
...
Homebrew로 설치한 경우 cMLX를 관리형 백그라운드 서비스로 실행할 수 있습니다:

```bash
brew services start cmlx    # 시작 (크래시 시 자동 재시작)
brew services stop cmlx     # 중지
brew services restart cmlx  # 재시작
brew services info cmlx     # 상태 확인
```

서비스는 기본 설정으로 `cmlx serve`를 실행합니다 (`~/.cmlx/models`, 포트 8000). 커스터마이즈하려면 환경 변수(`CMLX_MODEL_DIR`, `CMLX_PORT` 등)를 설정하거나, `cmlx serve --model-dir /your/path`를 한 번 실행하여 `~/.cmlx/settings.json`에 설정을 저장하세요.

로그는 두 곳에 기록됩니다:
- **서비스 로그**: `$(brew --prefix)/var/log/cmlx.log` (stdout/stderr)
- **서버 로그**: `~/.cmlx/logs/server.log` (구조화된 애플리케이션 로그)
...
<p align="center">
  <img src="docs/images/Screenshot 2026-02-10 at 00.45.34.png" alt="cMLX 관리자 대시보드" width="720">
</p>
...
<p align="center">
  <img src="docs/images/cmlx_hot_cold_cache.png" alt="cMLX Hot & Cold 캐시" width="720">
</p>
...
<p align="center">
  <img src="docs/images/cmlx_ChatTemplateKwargs.png" alt="cMLX 채팅 템플릿 파라미터" width="480">
</p>
...
<p align="center">
  <img src="docs/images/ScreenShot_2026-03-14_104350_610.png" alt="cMLX 채팅" width="720">
</p>
...
<p align="center">
  <img src="docs/images/downloader_cmlx.png" alt="cMLX 모델 다운로드" width="720">
</p>
...
<p align="center">
  <img src="docs/images/benchmark_cmlx.png" alt="cMLX 벤치마크 도구" width="720">
</p>
...
<p align="center">
  <img src="docs/images/Screenshot 2026-02-10 at 00.51.54.png" alt="cMLX 메뉴 바 통계" width="400">
</p>
...
모든 설정은 `/admin`의 웹 관리자 패널에서도 설정할 수 있습니다. 설정은 `~/.cmlx/settings.json`에 저장되며, CLI 플래그가 우선합니다.
...
- [vllm-mlx](https://github.com/waybarrios/vllm-mlx) - cMLX는 vllm-mlx v0.1.0에서 시작하여 멀티 모델 서빙, 계층형 KV 캐시, 페이지드 캐시를 완전 지원하는 VLM, 관리자 패널, macOS 메뉴 바 앱으로 크게 발전했습니다
