/*
 * cMLX Native Runtime — C++ Extension Module
 * ============================================
 * High-performance native code for the cMLX inference server.
...
int fast_cache_warmup(const char* file_path) {
    try {
        ...
        fprintf(stderr, "[cmlx_native] warmup failed for %s: %s\n",
                file_path, e.what());
        return 0;
    }
}
...
int parallel_warmup_dir(const char* dir_path, int num_threads) {
    try {
        ...
        fprintf(stderr, "[cmlx_native] parallel_warmup_dir failed: %s\n", e.what());
        return -1;
    }
}
...
FP8EncodeResult fp8_encode_tensor(const void* data, int n_elements, int dtype_code, unsigned char* out_fp8) {
    ...
    try {
        ...
        fprintf(stderr, "[cmlx_native] fp8_encode_tensor failed: %s\n", e.what());
    }
    return result;
}
...
int fp8_decode_tensor(const unsigned char* fp8_data, int n_elements,
    ...
    try {
        ...
        fprintf(stderr, "[cmlx_native] fp8_decode_tensor failed: %s\n", e.what());
        return 0;
    }
}
...
int native_save_safetensors(
    ...
    try {
        ...
        std::fclose(f);
        std::fprintf(stderr, "[cmlx_native] native_save_safetensors failed for %s: %s\n",
                     path, e.what());
        return 0;
    } catch (...) {
        std::fclose(f);
