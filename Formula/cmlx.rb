class Omlx < Formula
  ...
  homepage "https://github.com/shantibsharma/cmlx"
  url "https://github.com/shantibsharma/cmlx/archive/refs/tags/v0.3.5.dev1.tar.gz"
  sha256 "c9877d8070fcbe4a98ca687815be79cf9dea5392e74e4e88a4554e563fe7f4d3"
  license "Apache-2.0"

  head "https://github.com/shantibsharma/cmlx.git", branch: "main"
  ...
  # mlx-audio pins mlx-lm==0.31.1 which conflicts with cmlx's git-pinned
  ...
  service do
    run [opt_bin/"cmlx", "serve"]
    keep_alive true
    working_dir var
    log_path var/"log/cmlx.log"
    error_log_path var/"log/cmlx.log"
  ...
  def install
    ...
    # Install cmlx (with optional grammar extra for structured output)
    ...
    # python-multipart is declared in cmlx's [audio] extra, not in mlx-audio
    system libexec/"bin/pip", "install", "python-multipart>=0.0.5"

    bin.install_symlink Dir[libexec/"bin/cmlx"]
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/cmlx --version")
  end
end
