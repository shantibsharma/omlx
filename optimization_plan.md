...
## Context
Following the `ARCHITECTURE_AND_OPTIMIZATION.md` document, we are implementing a series of performance optimizations to `omlx` by moving critical management logic from Python into the existing C++ runtime. The primary goals are to prevent Metal OOM panics, reduce KV cache management latency, and minimize the Python-C++ boundary overhead during the generation loop.

The codebase already has a `src/` directory containing C++ core implementations (`cache_core.cpp`, `scheduler_core.cpp`, `omlx_fast_io.cpp`) which are integrated into Python via `ctypes` in `omlx/c_bindings.py`.
...
- **Task 1.1: Granular Memory Accounting in `src/cache_core.cpp`**
  ...
  - **Files:** `src/cache_core.cpp`, `omlx/c_bindings.py`, `omlx/cache/paged_cache.py`.
...
- **Task 1.2: Predictive Limit Management**
  ...
  - **Files:** `src/scheduler_core.cpp`, `omlx/scheduler.py`.
...
- **Task 2.1: Native O(1) LRU Management Optimization**
  ...
  - **Files:** `src/cache_core.cpp`, `omlx/cache/paged_cache.py`, `omlx/c_bindings.py`.
...
- **Task 2.2: Dynamic KV Cache Quantization (Research/Implementation)**
  ...
  - **Files:** `src/cache_core.cpp`, `omlx/cache/paged_cache.py`.
...
- **Task 3.2: Native Samplers & Logit Processors**
  ...
  - **Files:** `src/` (new or existing), `omlx/engine/`.
...
- Use `omlx serve` to start a server.
