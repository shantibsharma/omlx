...
"""
Tests for cMLX menubar app components (packaging/cmlx_app/).
...
from cmlx_app.config import ServerConfig, get_app_support_dir, get_config_path, get_log_path
from cmlx_app.server_manager import PortConflict, ServerManager, ServerStatus
...
class TestServerConfig:
    ...
    def test_default_values(self):
        ...
        config = ServerConfig()
        assert config.base_path == str(Path.home() / ".cmlx")
    ...
    def test_from_dict_partial_data(self):
        ...
        assert config.base_path == str(Path.home() / ".cmlx")  # default
    ...
    def test_save_and_load(self, tmp_path: Path):
        ...
        with patch("cmlx_app.config.get_config_path", return_value=config_file):
    ...
    @patch("cmlx_app.config.requests.Session")
...
class TestServerManager:
    ...
    @patch("cmlx_app.server_manager.requests.get")
    ...
    @patch("cmlx_app.server_manager.subprocess.Popen")
    @patch("cmlx_app.server_manager.get_log_path")
    ...
    def test_start_success(
        ...
        # Verify command includes cmlx.cli
        call_args = mock_popen.call_args
        cmd = call_args[0][0]
        assert "-m" in cmd
        assert "cmlx.cli" in cmd
        assert "serve" in cmd
    ...
    @patch("cmlx_app.server_manager.os.killpg")
    @patch("cmlx_app.server_manager.os.getpgid")
    ...
    def test_cleanup_dead_process(self, manager: ServerManager):
        ...
        with patch("cmlx_app.server_manager.os.killpg"), \
             patch("cmlx_app.server_manager.os.getpgid", return_value=12345):
    ...
    def test_try_auto_restart_resets_after_stable(self, mock_do_start, manager: ServerManager):
        ...
        with patch("cmlx_app.server_manager.os.killpg"), \
             patch("cmlx_app.server_manager.os.getpgid", return_value=123):
            manager._try_auto_restart("Server exited with code -9")
...
class TestPortConflict:
    ...
    def test_port_conflict_dataclass(self):
        ...
        conflict = PortConflict(pid=12345, is_cmlx=True)
        assert conflict.pid == 12345
        assert conflict.is_cmlx is True

        conflict_no_pid = PortConflict(pid=None, is_cmlx=False)
        assert conflict_no_pid.pid is None
        assert conflict_no_pid.is_cmlx is False

    @patch.object(ServerManager, "_find_port_owner_pid", return_value=12345)
    @patch.object(ServerManager, "_is_cmlx_server", return_value=True)
    @patch.object(ServerManager, "_is_port_in_use", return_value=True)
    def test_start_returns_port_conflict_cmlx(
        self, mock_port, mock_cmlx, mock_pid, manager: ServerManager
    ):
        """Test start returns PortConflict when port is used by cMLX."""
        result = manager.start()
        assert isinstance(result, PortConflict)
        assert result.pid == 12345
        assert result.is_cmlx is True

    @patch.object(ServerManager, "_find_port_owner_pid", return_value=99999)
    @patch.object(ServerManager, "_is_cmlx_server", return_value=False)
    @patch.object(ServerManager, "_is_port_in_use", return_value=True)
    def test_start_returns_port_conflict_not_cmlx(
        self, mock_port, mock_cmlx, mock_pid, manager: ServerManager
    ):
        ...
        assert result.is_cmlx is False

    @patch.object(ServerManager, "_is_cmlx_server", return_value=True)
    def test_adopt_success(self, mock_cmlx, manager: ServerManager):
        """Test adopting an existing cMLX server."""
    ...
    @patch.object(ServerManager, "_is_cmlx_server", return_value=False)
    def test_adopt_failure(self, mock_cmlx, manager: ServerManager):
        """Test adopt fails when target is not cMLX."""
        result = manager.adopt()
        assert result is False
    ...
    @patch("cmlx_app.server_manager.subprocess.run")
    ...
    @patch("cmlx_app.server_manager.os.kill")
...
class TestPathHelpers:
    ...
    @patch("cmlx_app.config.Path.home")
    def test_get_app_support_dir(self, mock_home, tmp_path):
        ...
        expected = tmp_path / "Library" / "Application Support" / "cMLX"
        assert result == expected
        assert expected.exists()

    @patch("cmlx_app.config.get_app_support_dir")
